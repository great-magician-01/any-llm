...
- Gateway completion requests additionally log: key id/name, upstream, model, in_format, stream flag, token usage, status
...
- `/api/admin/*` — admin API (HMAC session auth, cookie `s`)
  - CRUD for upstreams, models, ext keys, model aliases (`/api/admin/aliases`); usage summary/records. Ext keys carry a display `name` (required by the UI) plus a free-form `remark`; `GET /api/admin/usage/summary?group_by=key` reports the joined key `name` as `group_key` — falling back to `#<id>` for an empty name or a missing key row, then `—` when the record has no key id — while still grouping by `ext_key_id`, so two same-named keys stay separate rows and a renamed or soft-deleted key shows its current name (no name snapshot in `usage_records`)
