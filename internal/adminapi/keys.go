...
func (a *API) listKeys(w http.ResponseWriter, r *http.Request) {
	list, err := store.ListExtKeys(a.db)
	if err != nil {
...
func (a *API) createKey(w http.ResponseWriter, r *http.Request) {
	...
	if err := a.writeSync(func(d *sql.DB) error {
		var e error
		k, e = store.CreateExtKey(d, req.Label, req.Remark, req.DailyTokenLimit, req.MonthlyTokenLimit, allowed)
		return e
	}); err != nil {
...
func (a *API) deleteKey(w http.ResponseWriter, r *http.Request, id int64) {
	if err := a.writeSync(func(d *sql.DB) error { return store.DeleteExtKey(d, id) }); err != nil {
...
func (a *API) updateKey(w http.ResponseWriter, r *http.Request, id int64) {
	...
	cur, err := store.GetExtKeyByID(a.db, id)
	if err != nil {
	...
	if err := a.writeSync(func(d *sql.DB) error {
		return store.UpdateExtKey(d, id, label, remark, enabled, daily, monthly, allowed)
	}); err != nil {
		logger.Error("admin: update key DB write failed", "id", id, "err", err)
		writeSyncErr(w, 400, err)
		return
	}
	updated, err := store.GetExtKeyByID(a.db, id)
	if err != nil {
