...
func TestListLatestBalances(t *testing.T) {
	a, d := setupAPI(t)
	store.InsertBalanceSnapshot(d, &store.BalanceSnapshot{UpstreamID: 1, UpstreamName: "ds", Vendor: "deepseek", Payload: json.RawMessage(`{"kind":"balance","is_available":true,"balances":[{"currency":"CNY","total":"110.00","granted":"10.00","topped_up":"100.00"}]}`)})
	store.InsertBalanceSnapshot(d, &store.BalanceSnapshot{UpstreamID: 1, UpstreamName: "ds", Vendor: "deepseek", Payload: json.RawMessage(`{"kind":"balance","is_available":false,"balances":[]}`)})
	store.InsertBalanceSnapshot(d, &store.BalanceSnapshot{UpstreamID: 2, UpstreamName: "kimi", Vendor: "kimi-coding", Payload: json.RawMessage(`{"kind":"quota","windows":[{"id":"weekly","used_percent":"26.0"}]}`)})
...
func TestRefreshBalance_Unsupported(t *testing.T) {
	a, d := setupAPI(t)
	uid, _ := store.CreateUpstream(d, &store.Upstream{Name: "oai", BaseURL: "https://api.openai.com/v1", APIKey: "k", Format: "openai"})
