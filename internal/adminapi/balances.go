...
func (a *API) refreshAllBalances(w http.ResponseWriter, r *http.Request) {
	...
	snaps := make([]*store.BalanceSnapshot, len(upstreams))
	now := time.Now()
	...
	for i := range upstreams {
		...
		go func() {
			...
			snaps[i] = &store.BalanceSnapshot{UpstreamID: u.ID, UpstreamName: u.Name, Vendor: vendor, Payload: payload}
		}()
	}
	wg.Wait()
	...
	for _, snap := range snaps {
		...
		if err := a.writeSync(func(d *sql.DB) error { return store.InsertBalanceSnapshot(d, snap) }); err != nil {
