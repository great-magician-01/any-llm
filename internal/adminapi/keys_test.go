...
func TestCreateKeyInvalidJSON(t *testing.T) {
	...
	if list, _ := store.ListExtKeys(d); len(list) != 0 {
...
func TestKeyNameUnique(t *testing.T) {
	a, d := setupAPI(t)
	store.CreateExtKey(d, "taken", "", 0, 0, nil)
	...
	if list, _ := store.ListExtKeys(d); len(list) != 1 {
		t.Fatalf("rejected duplicate must not insert: len=%d", len(list))
	}

	// 改成占用中的名字：400，原名保留
	other, _ := store.CreateExtKey(d, "other", "", 0, 0, nil)
	body, _ = json.Marshal(map[string]any{"label": "taken"})
	...
	if got, _ := store.GetExtKeyByID(d, other.ID); got.Label != "other" {
...
func TestUpdateKeyLimits(t *testing.T) {
	...
	got, _ := store.GetExtKeyByID(d, k.ID)
...
func TestUpdateKeyNegativeLimitRejected(t *testing.T) {
	a, d := setupAPI(t)
	k, _ := store.CreateExtKey(d, "l", "", 100, 200, nil)
...
func TestUpdateKeyPartialNoChange(t *testing.T) {
	a, d := setupAPI(t)
	k, _ := store.CreateExtKey(d, "l", "keep-me", 100, 200, nil)
