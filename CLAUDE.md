...
**Config read cache** (`internal/model/cache.go`) — steps 1 and 3 read ext keys, upstream rows and alias binding chains through a process-local read-through cache (`CachedExtKey` / `CachedUpstreamByName` / `CachedAliasTargets`): first request queries the DB, every later request is served from memory, and hits return **copies** (the gateway shares the cache across goroutines). Token limits (step 4) are deliberately *not* cached — they aggregate `usage_records`, which is usage data, not config. Consistency is purely write-time: every config write in `internal/model` (`Create/Update/Delete` for keys, upstreams, aliases) refreshes or evicts the affected entries right after the DB write succeeds, inside the same `writeSync` closure, so an admin change is visible on the very next request. Because all admin CRUD and config import go through those functions, no caller can forget to invalidate — but it also means **mutating config out-of-band (raw SQL, or several processes sharing one DB) is invisible until restart**. Misses are never cached (otherwise arbitrary strings would grow the maps), alias candidates are stored *unfiltered* and expiry is evaluated at read time against the caller's `now`, and `TouchExtKey` intentionally does not invalidate (`last_used_at` drives nothing on the hot path).
...
- **Config read cache is process-local and has no TTL** — it is only ever refreshed by a write going through `internal/model`. If you ever edit `ext_keys` / `upstreams` / `model_aliases` directly in the DB (or run more than one process against the same DB), restart the process to pick the change up; there is no env var to disable the cache. Adding a config write? Put it in `internal/model` next to its siblings and invalidate there — not in `webapi`.
