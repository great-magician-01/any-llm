...
- **Dialect abstraction — one schema, three renderers** (`internal/db/schema.go` + `internal/db/dialect.go`): the canonical schema is a single `[]Table` of `Column`/`Index` values; per-dialect renderers emit `CREATE TABLE` / `CREATE INDEX` / `ALTER TABLE ADD COLUMN`. **Adding a column means editing that one definition** — the old `migrationSQLite` / `migrationPG` / `sqliteSoftDeleteSpecs` trio and its "keep these in sync" warning are gone. SQLite gets `INTEGER PRIMARY KEY AUTOINCREMENT` + `DATETIME`, PG `BIGSERIAL` + `TIMESTAMP(0)`, MySQL `BIGINT AUTO_INCREMENT` + `DATETIME(0)`.
