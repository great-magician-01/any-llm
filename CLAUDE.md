...
- **Config read cache is process-local** — it is refreshed by writes going through `internal/model` *and* by a fixed 1-hour TTL per entry (`configCacheTTL`), so an out-of-band DB edit (raw SQL, or several processes sharing one database) goes stale for at most an hour before the next read re-queries. There is no env var to disable it. Adding a config write? Put it in `internal/model` next to its siblings and invalidate there — not in `webapi`.
