...
`internal/gateway/router.go` routes the four public endpoints (`/v1/models`, `/v1/chat/completions`, `/v1/messages`, `/v1/responses`). `handler_openai.go` implements the pipeline:

1. **Ext-key auth** — `Authorization: Bearer all-sk-...` or `x-api-key` header; validated against DB and `Enabled` flag; `last_used_at` touched via async writer. `/v1/models` requires the same auth and lists only that key's allowed models.
2. **Model-permission check** — each key carries `allowed_models`, a whitelist of *public* model names (alias or `upstream/model` — exactly the ids `/v1/models` lists); empty = unrestricted, exact match only. An unlisted model → 403 `permission_error`, before alias resolution.
3. **Model resolve** — the request model is first exact-matched against **model aliases** (`model_aliases` + `model_alias_bindings`, fixed external names each bound to an ordered list of `upstream + real model`; aliases take precedence even for names containing `/`). On a miss it falls back to the `upstream-name/model-name` split on the first `/`; unknown upstream → 404. Alias candidates beyond the first provide automatic failover: the dispatch loop tries each in `priority` order (non-stream: until one succeeds; stream: until one call returns, since headers/keep-alives are already flowing — once content frames flow there is no failover). Candidates over their upstream token limit, or pointing at **disabled** upstreams, are skipped *before* dispatch (a disabled upstream on the direct route 404s with "is disabled", and `/v1/models` omits it — disable keeps models/bindings alive for re-enable, unlike delete). Upstreams can also carry an **expiry** (`expires_at`, nullable, read-time check via `store.Upstream.Expired` — no background job); an expired upstream is treated exactly like a disabled one (direct route 404s "is expired", candidates drop out of the chain, `/v1/models` omits it) but `enabled` is left untouched, so extending the date restores service without re-enabling. Usage records store the **actual** upstream model of each attempt.
4. **Token-limit check** — daily/monthly quotas on both the ext key and the upstream (aggregated from `usage_records` over local-day/local-month windows; `0` = unbounded), exceeded → 429.
5. **Dispatch** — decode inbound → IR, call upstream, encode response back. Each candidate first acquires a slot on its upstream's concurrency semaphore (`internal/gateway/concurrency.go`, capacity = the upstream's `max_concurrent`, default 100, 0 = unlimited); a full candidate is skipped, all-full → 429 `rate_limit_error` (streams get it cleanly before headers flush). Errors are written in the *client's* format (`WriteError`/`mapErrorType` in `errors.go`).
...
**Config read cache** (`internal/store/cache.go`) — steps 1 and 3 read ext keys, upstream rows and alias binding chains through a process-local read-through cache (`CachedExtKey` / `CachedUpstreamByName` / `CachedAliasTargets`): first request queries the DB, every later request is served from memory, and hits return **copies** (the gateway shares the cache across goroutines). Each entry also carries a **fixed 1-hour TTL** (`configCacheTTL`): on expiry the entry is treated as a miss — re-query, re-store, reset the timer — so a missed invalidation (or an out-of-band DB edit) is stale for at most an hour instead of forever. A failed re-query keeps the old entry and lets the next request retry. Concurrent misses are **single-flighted**: only one goroutine per key actually queries (the rest wait on an in-flight flight and then take the leader's broadcast result — negatives included, so a burst of direct-route alias probes costs one query), so a TTL boundary or cold start doesn't turn into N queries; the leader reports its own DB error while waiters retry. The re-fill is **guarded twice**: it records the entry it saw before querying plus a per-namespace write **generation** that every write-path function bumps (even for no-op evictions and wholesale namespace resets, which leave no visible entry change), and only stores if both are unchanged, so a config write landing mid-query can't be overwritten by the older row the query returned. Token limits (step 4) are deliberately *not* cached — they aggregate `usage_records`, which is usage data, not config. Consistency is primarily write-time: every config write in `internal/store` (`Create/Update/Delete` for keys, upstreams, aliases) refreshes or evicts the affected entries right after the DB write succeeds, inside the same `writeSync` closure, so an admin change is visible on the very next request. Because all admin CRUD and config import go through those functions, no caller can forget to invalidate. Misses are never cached (otherwise arbitrary strings would grow the maps), alias candidates are stored *unfiltered* and their `expires_at` is evaluated at read time against the caller's `now` (unrelated to the entry TTL), and `TouchExtKey` intentionally does not invalidate (`last_used_at` drives nothing on the hot path).
...
- **Dialect abstraction — one schema, three renderers** (`internal/db/schema.go` + `internal/db/dialect.go`): the canonical schema is a single `[]Table` of `Column`/`Index` values; per-dialect renderers emit `CREATE TABLE` / `CREATE INDEX` / `ALTER TABLE ADD COLUMN`. **Adding a column means editing that one definition** — flag it `LateAdd: true` if it postdates the initial schema so old databases get the `ALTER` (the flag is the only registry; there is no second list to keep in sync). All declared indexes are likewise created on every dialect: partial-unique ones via `migratePartialUniqueIndexes`, plain ones via `migratePlainIndexes`, both rendered from the same declaration (MySQL inlines them in `CREATE TABLE`). The old `migrationSQLite` / `migrationPG` / `sqliteSoftDeleteSpecs` trio and its "keep these in sync" warning are gone. SQLite gets `INTEGER PRIMARY KEY AUTOINCREMENT` + `DATETIME`, PG `BIGSERIAL` + `TIMESTAMP(0)`, MySQL `BIGINT AUTO_INCREMENT` + `DATETIME(0)`.
...
Vue 3 + TypeScript + Naive UI + Vue Router (hash history) + Axios. Two symmetric page sets live under `src/themes/`: `themes/classic/` (views + `Layout.vue` + naive `theme.ts`) and `themes/glass/` (the same pages under the `/glass` route prefix); `src/components/` keeps only components shared by both sets. `src/api/` has typed API modules with a shared axios instance whose response interceptor redirects to login on 401. Imports use the `@/` alias for `src/`. Vite dev server proxies `/api` and `/v1` to `localhost:6718`.

**明暗主题**（`src/composables/useTheme.ts`）：解析优先级 localStorage `any-llm-theme` > `prefers-color-scheme` > 暗色。主题挂在 `<html data-theme="light|dark">` 上，`style.css` 的 `:root` 仍是暗色默认值，浅色靠 `:root[data-theme='light']` 覆盖变量；`App.vue` / `themes/glass/GlassShell.vue` 按同一个状态在 naive-ui 的 `darkTheme` / `lightTheme` 与两套 `themeOverrides` 之间切换（`src/themes/classic/theme.ts`、`src/themes/glass/theme.ts`，两套只差表面/文字/边框，品牌色共用）。`index.html` 有一段等价的内联脚本负责首帧前预置 `data-theme` 防闪屏，改解析顺序要同步改它。切换入口是 `src/components/ThemeToggle.vue`（两个 Layout 侧栏 + 两个登录页）。注意 watcher 用 `flush: 'sync'`，且只有用户主动切换才写 localStorage（否则系统偏好会被固化）。
...
- **Timestamps are server-local everywhere**: write `time.Now()` (never `.UTC()`); PG `TIMESTAMP(0)` columns store the bare local wall clock and `OpenPG`'s custom `timestamp` codec (`internal/db/pgtime.go`) relabels scans from UTC back to `time.Local`, so JSON emits the local offset. SQLite stores RFC3339-with-offset text and needs no help. MySQL uses `DATETIME(0)` with DSN `parseTime=true&loc=Local`, so the driver reads/writes the local wall clock directly — no codec needed. `response_sessions.messages` must be `LONGTEXT` on MySQL (its `TEXT` caps at 64 KiB, and that column holds the whole accumulated session history).
- No rate-limiting middleware and no CORS middleware — the only traffic shaping is the per-upstream concurrency cap above.
- **Config read cache is process-local** — it is refreshed by writes going through `internal/store` *and* by a fixed 1-hour TTL per entry (`configCacheTTL`), so an out-of-band DB edit (raw SQL, or several processes sharing one database) goes stale for at most an hour before the next read re-queries. There is no env var to disable it. Adding a config write? Put it in `internal/store` next to its siblings and invalidate there — not in `adminapi`.
